...
def main(root_path, split, keepclasses, ratio):
    dirname = "BDD"

    # classnames_all = PASCAL_VOC_ALL_CATEGORIES[sid]
    if keepclasses == 'all':
        classnames = BDD_ALL_CATEGORIES[1]
    elif keepclasses == 'base':
        classnames = BDD_BASE_CATEGORIES[1]
    elif keepclasses == 'novel':
        classnames = BDD_NOVEL_CATEGORIES[1]

    fileids = {}
    for cls in classnames:
        with open(os.path.join("bddsplit/seed0", "box_{}ratio_{}_train.txt".format(ratio, cls))) as f:
    ...
    support_path = os.path.join(root_path, 'bdd_{}_{}_{}shot'.format(split, keepclasses, 1000))
...
if __name__ == '__main__':
    ...
    for keepclasses in ['all',]: #'novel']:
            for ratio in [0.001]:
                print("*******************keepclasses={}, ratio={}".format(keepclasses, ratio))

                since = time.time()
                support_df = main(root_path, split, keepclasses, ratio)
                support_df.to_pickle("./bdd_{}_{}_{}shot.pkl".format(split, keepclasses, 1000))
