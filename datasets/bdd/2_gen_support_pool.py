...
def main(root_path, split, keepclasses):
    dirname = "BDD"
    ...
    support_path = os.path.join(root_path, 'bdd_{}_{}'.format( split, keepclasses))
    ...
    for img_id, fileid in enumerate(fileids):
        if img_id % 100 == 0:
            print(img_id)
        anno_file = os.path.join(root_path, dirname, "Annotations", fileid + ".xml")
        jpeg_file = os.path.join(root_path, dirname, "JPEGImages", fileid + ".jpg")
...
if __name__ == '__main__':
    ...
    since = time.time()
    support_df = main(root_path, split, keepclasses)
    support_df.to_pickle("bdd_{}_{}.pkl".format(split, keepclasses))
