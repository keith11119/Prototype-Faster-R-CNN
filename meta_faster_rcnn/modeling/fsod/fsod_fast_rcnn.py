...
class FsodFastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.aligned_sub = True

        self.fewx_baseline = fewx_baseline
        if self.fewx_baseline:
            self.aligned_sub = False
        ...
        if self.aligned_sub:
            self.conv_1_gd = nn.Conv2d(dim_in, int(dim_in/4), 1, padding=0, bias=False)
            self.conv_2_gd = nn.Conv2d(int(dim_in/4), int(dim_in/4), 3, padding=0, bias=False)
            self.conv_3_gd = nn.Conv2d(int(dim_in/4), dim_in, 1, padding=0, bias=False)
            self.bbox_pred_gd = nn.Linear(dim_in, 4) #num_bbox_reg_classes * box_dim)
            self.cls_score_gd = nn.Linear(dim_in, 2) #nn.Linear(dim_in, 2)

        if self.local_correlation:
            self.conv_cor = nn.Conv2d(dim_in, dim_in, 1, padding=0, bias=False)
            if not self.fewx_baseline:
        ...
        if not self.fewx_baseline:
            self.norm = nn.LayerNorm([2048, 7, 7])
        ...
        if self.aligned_sub:
            nn.init.normal_(self.conv_1_gd.weight, std=0.01)
            nn.init.normal_(self.conv_2_gd.weight, std=0.01)
            nn.init.normal_(self.conv_3_gd.weight, std=0.01)
            nn.init.normal_(self.cls_score_gd.weight, std=0.01)
            nn.init.constant_(self.cls_score_gd.bias, 0)
            nn.init.normal_(self.bbox_pred_gd.weight, std=0.001)
            nn.init.constant_(self.bbox_pred_gd.bias, 0)
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "gamma_num"             : cfg.MODEL.ROI_BOX_HEAD.GAMMA_NUM,
            "fewx_baseline"         : cfg.MODEL.FEWX_BASELINE,
            "with_alignment"        : cfg.MODEL.WITH_ALIGNMENT
            # fmt: on
        }
    ...
    def forward(self, x_query, x_support):
        ...
        if self.with_alignment:
            support_full = support.expand_as(x_query)
            support_k = support_full.view(support_full.shape[0], support_full.shape[1], -1)

            x_query_q = x_query.view(x_query.shape[0], x_query.shape[1], -1)
            x_query_q = x_query_q.permute(0,2,1).contiguous()

            support_v = support_k.permute(0,2,1).contiguous()

            f = torch.matmul(x_query_q, support_k)
            N = f.size(-1)
            f_div_C = f / N

            f_sum = torch.sum(f, 2)
            mask = nn.Sigmoid().cuda()(f_sum)
            mask = mask.view(mask.shape[0], 1, x_query.shape[2], x_query.shape[3])

            aligned_support = torch.matmul(f_div_C, support_v)
            aligned_support = aligned_support.permute(0,2,1).contiguous()
            aligned_support = aligned_support.view(support_full.shape[0], support_full.shape[1], x_query.shape[2], x_query.shape[3])

            if self.gamma_num == 0 or self.gamma_num == 1:
                support = self.gamma * aligned_support * mask + support_full
                x_query = self.gamma * x_query * mask + x_query
            elif self.gamma_num == 2:
                support = self.gamma[0] * aligned_support * mask + support_full
                x_query = self.gamma[1] * x_query * mask + x_query

            self.invoke_count += 1
            if self.invoke_count % 1000 == 0:
                print("support.shape=", support.shape)
                print("self.gamma=", self.gamma)

        if not self.fewx_baseline:
            support = self.norm(support)
        ...
        if self.aligned_sub:
            y = x_query - support
            y = F.relu(self.conv_1_gd(y), inplace=True) # 5x5
            y = self.avgpool(y)
            y = F.relu(self.conv_2_gd(y), inplace=True) # 3x3
            y = F.relu(self.conv_3_gd(y), inplace=True) # 3x3
            y = self.avgpool(y) # 1x1
            y = y.squeeze(3).squeeze(2)
            cls_score_gd = self.cls_score_gd(y)
        ...
        if self.local_correlation:
            ...
            if self.with_alignment:
                support_full = support.expand_as(x_query)
                support_cor = self.conv_cor(support_full)
                x_cor = []
                for idx in range(x_query_cor.shape[0]):
                    x_cor.append(F.relu(F.conv2d(x_query_cor[idx, :].unsqueeze(0), support_cor[idx, :].unsqueeze(0).permute(1,0,2,3), groups=2048), inplace=True).squeeze(3).squeeze(2))
                x_cor = torch.cat(x_cor, dim=0)
            else:
        ...
        if self.fewx_baseline:
            ...
            bbox_pred_all = self.bbox_pred_pr(x) + self.bbox_pred_gd(y) + self.bbox_pred_cor(x_cor) + self.bbox_pred_fc(out_fc)
            cls_score_all = cls_score_pr + cls_score_cor + cls_score_gd + cls_score_fc
